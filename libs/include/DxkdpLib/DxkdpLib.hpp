...
private:
    /**
     * @brief Sets up Serial Port for usage. Call before any write/read operation.
     * 
     */
    void DXKDP_Setup();
    ...
    */
    ...
    /**
     * @brief float to 16bit Hex, writes Low and High bit in the input_msg struct direclty.
     * 
...
public:
    ...
    /**
     * @brief Default Constructor. Instantiates supply at /dev/ttyUSB0, addr=0x01
     * 
     */
    DXKDP_PSU();
    ...
    /**
     * @brief Overloaded Constructor, allows to choose COM PORT and Conversion Factors
     * 
     * @param COM_PORT string containing COM Port Address
     ...
     * 
     * Accesses msgIn.instruction to do the writing. Set it through the Encoder functions.
     * 
     ...
     * 
     * Writes the read data onto the output1 member of the output_message struct.
     * 
     * @param msgOut reference to the output_message struct.
     */
     ...
     * 
     * Mixed polarities are unachievable as of now, so this function does not allow them.
     * 
