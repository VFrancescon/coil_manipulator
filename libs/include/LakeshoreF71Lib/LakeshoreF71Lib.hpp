...
private:
     ...
     * 
     * Sets sensing range to Auto and units to Tesla. Call this before attempting any reading.
     */
    void TeslameterSetup();

    std::string autoRange   = "SENS:FIEL:RANG:AUTO 1"; //!< Command to set the range to automatic

    
    std::string fieldTesla  = "UNIT:FIEL TESL"; //!< Command to set the field to tesla
    
    std::string fetchField  = "FETC:FIEL:DC? "; //!< Command to fetch the field in a direction. MUST ADD X/Y/Z/ALL to complete command
    ...
    std::string lineFeed    = "\n\0"; //!< Need to attach a line feed to each command to complete them
...
public:
    ...
    /**
     * @brief Returns field reading in axis Axis. Look at enum class AXIS for details
     * 
     ...
     * 
     * @return std::vector<float> Vector containing X, Y and Z wise readings, in that order
     */
